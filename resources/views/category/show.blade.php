        ...
        @foreach ($products as $product)
            @php
                ...
                if (is_string($images)) {
                    ...
                    $images = json_last_error() === JSON_ERROR_NONE && is_array($decoded) ? $decoded : [];
                }

                $badgeLabels = $product->badge_labels ?? [];
                if (is_string($badgeLabels)) {
                    $decoded = json_decode($badgeLabels, true);
                    $badgeLabels = json_last_error() === JSON_ERROR_NONE && is_array($decoded) ? $decoded : [];
                }
                $badgeLabels = is_array($badgeLabels) ? $badgeLabels : [];

                $statusBadges = $product->status_badges ?? [];
                if (is_string($statusBadges)) {
                    $decoded = json_decode($statusBadges, true);
                    $statusBadges = json_last_error() === JSON_ERROR_NONE && is_array($decoded) ? $decoded : [];
                }
                $statusBadges = is_array($statusBadges) ? $statusBadges : [];
                                ...
                                <div
                                    ...
                                    <div class="card-product__group-item card-wishlist">
                                        <a class="wishlist-icon" href="#"
                                            data-wishlist=""
                                            data-wishlist-handle="{{ $product->shopify_handle ?? $product->slug }}"
                                            data-product-id="{{ $product->shopify_id ?? $product->id }}" role="button">
                                    ...
                                    <div
                                        ...
                                        <a class="quickview-icon" href="javascript:void(0)"
                                            ...
                                            data-product-handle="{{ $product->shopify_handle ?? $product->slug }}"
                                            role="button">
                                    ...
                                    <div class="card-product__group-item card-compare card-compareIcon"
                                        ...
                                        data-product-compare-handle="{{ $product->shopify_handle ?? $product->slug }}"
                                        data-product-compare-id="{{ $product->shopify_id ?? $product->id }}">
                                        <div class="compare-icon">
                                            <input
                                                id="compare-{{ $product->shopify_id ?? $product->id }}-template--17315346808947__product-grid"
                                                class="compare-checkbox" type="checkbox"
                                                name="compare"
                                                value="{{ $product->shopify_handle ?? $product->slug }}">
                                            <label class="compare-label"
                                                for="compare-{{ $product->shopify_id ?? $product->id }}-template--17315346808947__product-grid">
